...
def main():
    ...
    print(
        f'{"ID":<5} {"Name":<40} {"Order":<7} {"DocClass":<15} {"PostBranch":<12} {"Branching":<10}'
    )
    print("-" * 100)
    ...
    for step in all_enabled:
        ...
        print(
            f"{step.id:<5} {step.name:<40} {step.order:<7} {doc_class:<15} {str(step.post_branching):<12} {str(step.is_branching_step):<10}"
        )
    ...
    if universal_steps:
        for step in universal_steps:
            print(
                f"ID: {step.id}, Name: {step.name}, Order: {step.order}, PostBranching: {step.post_branching}"
            )
    else:
        print("❌ NO UNIVERSAL STEPS FOUND!")
        print("\n🔍 Root Cause Analysis:")
        print(
            "   - get_universal_steps() returns 0 because ALL enabled steps have document_class_id set"
        )
        ...
        print(
            "   - Post-branching universal steps must have document_class_id = NULL AND post_branching = True"
        )
