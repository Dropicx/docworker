...
def list_user_keys(user_email):
    ...
    try:
        ...
        for key in keys:
            ...
            print(
                f"{key.id} {key.name:<20} {key.is_active!s:<8} {expires_str:<20} {last_used_str:<20}"
            )
...
def list_all_keys():
    ...
    try:
        ...
        for key in keys:
            expires_str = key.expires_at.isoformat() if key.expires_at else "Never"
            print(
                f"{key.id} {key.user.email:<30} {key.name:<15} {key.is_active!s:<8} {expires_str:<20}"
            )
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Manage DocTranslator API keys")
