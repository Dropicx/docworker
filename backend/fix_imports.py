    ...
    try:
        ...
        if needs_timedelta and datetime_import_idx >= 0:
            if "timedelta" not in lines[datetime_import_idx]:
                lines[datetime_import_idx] = (
                    lines[datetime_import_idx]
                    .replace("from datetime import ", "from datetime import timedelta, ")
                    .replace("datetime, timedelta", "timedelta, datetime")
                )  # Keep datetime first
        ...
        if (needs_request or needs_header) and fastapi_import_idx >= 0:
            ...
            if imports_to_add:
                ...
                if "(" in lines[fastapi_import_idx]:
                    ...
                    # Single line import
                    lines[fastapi_import_idx] = lines[fastapi_import_idx].replace(
                        "from fastapi import ", f"from fastapi import {', '.join(imports_to_add)}, "
                    )
