...
def test_order_behavior():
    ...
    with engine.connect() as conn:
        ...
        for run in range(1, 6):
            ...
            for row in rows:
                print(
                    f"  ID {row[0]:<3} | Order {row[2]:<3} | post_branching={row[3]:<6} | {row[1]}"
                )

        print("\n" + "=" * 120)
        print(
            "Test 2: Check if row order changes between runs (deterministic vs non-deterministic)"
        )
        print("-" * 120)
        ...
        print("\n" + "=" * 120)
        print(
            "Test 5: What if Language Translation (post_branching=TRUE) is returned BEFORE Medical Validation?"
        )
        print("-" * 120)
