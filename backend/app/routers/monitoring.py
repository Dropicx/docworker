...
def get_queue_lengths():
    ...
    try:
            ...
            for key in possible_keys:
                try:
                    ...
                    if key_len > 0:
                        length = key_len
                        logger.debug(
                            f"   Found queue '{queue_name}' at key '{key}': {length} tasks"
                        )
                        break
                except Exception:
                    continue
        ...
        return {"high_priority": 0, "default": 0, "low_priority": 0, "maintenance": 0}
...
async def redis_debug():
    ...
    try:
        ...
        for key in all_keys:
            ...
            key_type = (
                redis_client.type(key).decode()
                if isinstance(redis_client.type(key), bytes)
                else redis_client.type(key)
            )
            ...
            if key_type == "list":
                ...
                key_details[key_str] = {"type": "string", "value": redis_client.get(key)}
            elif key_type == "hash":
                key_details[key_str] = {
                    "type": "hash",
                ...
                key_details[key_str] = {"type": key_type}
        ...
        return {
            ...
            "queue_patterns": [
                ...
                if any(q in k for q in ["high_priority", "default", "low_priority", "maintenance"])
            ],
        }
    except Exception as e:
...
async def flower_status():
    ...
    try:
        ...
        return {"available": False, "flower_url": FLOWER_URL_PUBLIC, "error": "Connection refused"}
    except Exception as e:
        logger.error(f"❌ Error checking Flower status: {str(e)}")
        return {"available": False, "flower_url": FLOWER_URL_PUBLIC, "error": str(e)}
...
async def worker_stats():
    ...
    try:
        ...
        if workers_response.status_code == 200 and tasks_response.status_code == 200:
            ...
            return {
                "workers": {"total": total_workers, "active": active_workers, "details": workers},
                "tasks": {
                    "total": len(tasks),
        ...
        raise HTTPException(status_code=503, detail="Failed to fetch worker statistics from Flower")
