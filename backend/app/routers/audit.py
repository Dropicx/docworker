...
async def list_audit_logs(
    ...
    try:
        ...
        return AuditLogListResponse(
            logs=log_responses, total=len(log_responses), page=page, limit=limit
        )

    except Exception as e:
        logger.error(f"Error listing audit logs: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to list audit logs"
        ) from e
...
async def get_ip_audit_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get IP audit logs"
        ) from e
...
async def get_audit_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get audit summary"
        ) from e
...
async def export_audit_logs_csv(
    ...
    try:
        ...
        # Get CSV data
        csv_data = audit_repo.export_logs_csv(start_date=start_date, end_date=end_date, limit=limit)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to export audit logs"
        ) from e
...
async def cleanup_old_audit_logs(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_user.email} cleaned up {count} old audit logs (kept {days} days)"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to cleanup audit logs"
        ) from e
...
async def audit_health_check():
    ...
    Returns:
        Service status
    """
    return {"status": "healthy", "service": "audit", "version": "1.0.0"}
