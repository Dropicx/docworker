...
class ImprovedTableProcessor:
    ...
    def process_ocr_output(
        self, text: str, ocr_data: dict | None = None, use_advanced: bool = True
    ) -> str:
        """
        ...
        return self._enhance_medical_content(result)
    ...
    def _is_parameter_value_pair(self, text1: str, text2: str) -> bool:
        ...
        return bool(re.match("^[\\d,.\\-]+\\s*\\w*$", text2))
    ...
    def _looks_like_lab_result(self, parts: list[str]) -> bool:
        ...
        might_be_param = (
            len(first_part_lower) > 1
            and not first_part_lower.isdigit()
            and not all(c in ".-_=><" for c in first_part_lower)
        )
    ...
    def _is_noise_line(self, line: str) -> bool:
        ...
        return bool(alphanum_chars > 0 and special_chars / alphanum_chars > 3)
    ...
    def _enhance_medical_content(self, text: str) -> str:
        ...
        return self._add_section_headers(text)
    ...
    def _add_section_headers(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if not has_lab_values and any(
                param in line_lower
                for params in self.medical_parameters.values()
                for param in params
            ):
            ...
            if not has_diagnosis and any(
                term in line_lower for term in ["diagnose", "befund", "beurteilung"]
            ):
                enhanced_lines.append("\n=== DIAGNOSE/BEFUND ===")
                has_diagnosis = True
    ...
    def _advanced_row_reconstruction(self, text: str, ocr_data: dict) -> str:
        ...
        try:
            ...
            # Clean up the result
            return self._post_process_reconstruction(result)
        ...
        for word in row_words:
            if prev_word:
                ...
                if gap > 100:  # Very large gap - likely different columns
                    ...
                    result.append("  ")
                elif gap > 15 or gap > 0:  # Normal word spacing
                    result.append(" ")
    ...
    def _post_process_reconstruction(self, text: str) -> str:
        ...
        for line in lines:
            ...
            line = re.sub(
                r"\b([A-Za-z]+)\s+([0-9])\s+([0-9])\b", r"\1\2\3", line
            )  # Join split numbers
